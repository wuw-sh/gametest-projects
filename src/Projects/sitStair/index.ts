...
world.events.itemUseOn.subscribe(data => {
    ...
    const testEt = !executeCommand(pl, `testfor @e[type=sit:sit,c=1,tag=${pl.name}]`).err
    ...
    executeCommand(pl, `ride @s start_riding @e[type=sit:sit,c=1,tag=${pl.name}] teleport_rider`)
})
